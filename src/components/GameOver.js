...
export default class GameOver extends Phaser.Scene {
  ...
  create() {
    ...
    const textAgain = this.make.text(FONT_PROPS(CONST.TEXT.AGAIN));
    const title = this.make.text(
      FONT_PROPS(this.registry.get('new') == true ? CONST.TEXT.NEW_RECORD : CONST.TEXT.CHEERUP, 48),
    );
    const textScore = this.make.text(
      FONT_PROPS(
        `${this.registry.get('new') == true ? CONST.TEXT.NEW_SCORE : CONST.TEXT.BEST_SCORE} ${localStorage.highscore}`,
      ),
    );

...
export default class GameOver extends Scene {
  ...
  createCubefall() {
    ...
    for (let i = 0; i < 20; i += 1) {
      const color = Math.Between(0, FRAMES.length - 1);
      const sx = Math.Between(CUBE_WIDTH, GAME_WIDTH);
      const sy = Math.Between(0, GAME_HEIGHT);
      const block = this.add.sprite(sx, sy, 'sprites_2', CONST.FRAMES[color]).setScale(0.3).setAlpha(0.7);
      this.arr[i] = block;
    }
  }
  ...
  create() {
    ...
    Display.Align.In.QuickSet(title, screenCenter, 6, 0, 0);
    Display.Align.In.QuickSet(textScore, screenCenter, 6, 0, 50);
    Display.Align.In.QuickSet(textAgain, screenCenter, 6, 0, 300);
  ...
  update() {
    this.arr.forEach((arrayItem) => {
      const block = arrayItem;
      block.y += 0.5;
